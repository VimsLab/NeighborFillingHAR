...
class P4DConv(nn.Module):
    ...
    def forward(self, xyzs: torch.Tensor, features: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        ...
        new_xyzs = []
        new_features = []
        neighbor_xyz_groupeds = []
        anchor_xyz_expandeds = []
        neighbor_xyz_groupeds_ball = []
        inputs = []
        for t in range(self.temporal_kernel_size//2, len(xyzs)-self.temporal_kernel_size//2, self.temporal_stride):                 # temporal anchor frames
            ...
            for i in range(t-self.temporal_kernel_size//2, t+self.temporal_kernel_size//2+1):
                neighbor_xyz = xyzs[i]
                idx = pointnet2_utils.ellipsoid_query(self.r, self.k, neighbor_xyz, anchor_xyz)#
                ...
                neighbor_xyz_groupeds.append(neighbor_xyz_grouped)

                xyz_displacement = (neighbor_xyz_grouped - anchor_xyz_expanded)/(2*self.r)# (B, 3, N//spatial_stride, k)
